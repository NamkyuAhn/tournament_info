...
from tournaments.models import Tournament, TournamentEntry, PokerTournament, BuyInEvent
...
class TournamentBuyInService:
    ...
    def _handle_entry(user, tournament):
        ...
        entry = (
            ...
            .filter(player=user, tournament=tournament)
            .first()
        )
        ...
        TournamentBuyInService._validate_entry_capacity(tournament)
        ...
        BuyInEvent.objects.create(
            entry=entry, type=BuyInEvent.TypeChoices.ENTRY, amount=tournament.entry_fee
        )
    ...
    def _handle_reentry(user, tournament):
        ...
        if tournament.game_type != (Tournament.GameTypeChoices.POKER):
            ...
            raise ValidationError("Reentry is only available for poker tournaments.")

        if tournament.status != (Tournament.StatusChoices.RUNNING):
        ...
        entry = TournamentBuyInService._get_entry(user, tournament)

        if entry.status != (TournamentEntry.StatusChoices.BUSTED):

            raise ValidationError("Reentry allowed only after bust.")

        if entry.total_reentries_cache >= poker_tournament.max_reentries:
    ...
    def _handle_addon(user, tournament):
        ...
        if tournament.game_type != (Tournament.GameTypeChoices.POKER):
            ...
            raise ValidationError("Addon is only available for poker tournaments.")

        if tournament.status != (Tournament.StatusChoices.RUNNING):

            raise ValidationError("Addon allowed only during running tournament.")
    ...
    def _validate_entry_capacity(tournament):
        ...
        if tournament.game_type == (Tournament.GameTypeChoices.POKER):
            ...
            if tournament.live_players_cache >= tournament.max_participants:
                ...
                raise ValidationError("Tournament participant limit reached.")
    ...
    def _get_entry(user, tournament):
        ...
        try:
            ...
            return TournamentEntry.objects.select_for_update().get(
...
class TournamentPlayerManageService:
    ...
    def get_my_tournaments(user):
        ...
        return (
            Tournament.objects.filter(entries__player=user)
            .select_related("shop")
            .order_by("-start_time")
            .distinct()
        )
    ...
    @staticmethod
    def get_my_tournament_detail(user, tournament_id):
        ...
        entry = (
            ...
            .filter(player=user, tournament_id=tournament_id)
            .first()
        )
