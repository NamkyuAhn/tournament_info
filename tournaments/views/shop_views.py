...
from tournaments.views.request_data import decode_tournament_json_fields
...
class TournamentCreateView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        data = decode_tournament_json_fields(dict(request.data.items()))
...
class TournamentEditView(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        data = decode_tournament_json_fields(request.data.dict())

        existing_image_ids = None

        if "existing_image_ids" in data:

            existing_image_ids = json.loads(data["existing_image_ids"])

            data.pop("existing_image_ids", None)
...
class ShopTournamentListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return Tournament.objects.filter(shop__owner=self.request.user).order_by(
            "-created_at"
        )
    ...
    def get_queryset(self):
        ...
        return Tournament.objects.filter(
            ...
            Prefetch(
                "entries",
                queryset=(
                    TournamentEntry.objects.select_related("player").prefetch_related(
                        "buyin_events"
                    )
                ),
            ),
...
class TournamentStatusUpdateView(APIView):
    ...
    def patch(self, request, tournament_id):
        ...
        tournament = get_object_or_404(
            Tournament, id=tournament_id, shop__owner=request.user
        )
        ...
        valid_statuses = dict(Tournament.StatusChoices.choices)
        ...
        if tournament.status == Tournament.StatusChoices.CANCELED:
            return Response(
                {"detail": ("Canceled tournaments cannot change status.")},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class TournamentCancelView(APIView):
    ...
    def post(self, request, tournament_id):
        ...
        TournamentPlayerManageService.cancel_tournament(
            owner=request.user, tournament_id=tournament_id
        )

        return Response(
            {"detail": ("Tournament canceled successfully.")}, status=status.HTTP_200_OK
        )
